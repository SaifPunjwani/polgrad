...
def test_golden_case_hand_derived() -> None:
    ...
    moments, quantiles, k1/k2/k3 both directions, sequence stats, catastrophic
    indices at gap 0.7, and ppl_ratio = exp(-0.2)."""
    ...
    # k1 = mean Δ; k2 = mean Δ²/2; k3 = mean exp(-Δ) - 1 + Δ; reversed k3 swaps sign.
    ...
    assert report.kl_k3_reversed == pytest.approx(expected_k3_rev, rel=1e-9)
...
def test_identical_streams_zero_gaps() -> None:
    ...
    assert report.kl_k3_reversed == 0.0
...
def test_kl_k1_equals_gap_mean_and_ppl_ratio_is_exp_neg(batch) -> None:  # type: ignore[no-untyped-def]
    """Internal consistency: k1 of KL(trainer‖rollout) is the mean gap, and
...
def test_k3_estimates_nonnegative(batch) -> None:  # type: ignore[no-untyped-def]
    """k3 = exp(δ) - 1 - δ ≥ 0 pointwise, so both direction estimates are nonnegative
    (docs/derivations/kl.md), as is k2."""
    report = logprob_mismatch(batch.old_logprobs, batch.rollout_logprobs, batch.response_mask)
    assert report.kl_k2 >= 0.0
    assert report.kl_k3 >= -1e-12
    assert report.kl_k3_reversed >= -1e-12
