...
ALGORITHMS: dict[str, AlgorithmSpec] = {
    ...
    "dr_grpo": AlgorithmSpec(
        ...
        notes=(
            ...
            "Sampling temperature 1.0 is stated in Table 6. The paper does not state the +eps "
            "std guard; its released code pins 1e-8 (train_zero_math.py), unused here since "
            "scale='none'."
        ),
    ),
    "dapo": AlgorithmSpec(
    ...
    "gspo": AlgorithmSpec(
        ...
        notes=(
            ...
            "4e-4' - roughly 600-700x (nearly three orders of magnitude) tighter than "
            "token-level PPO clipping because the length-normalized sequence ratio "
            "concentrates near 1 (the same paper runs its GRPO baseline at 0.2/0.27; the "
            "paper's own 'two orders of magnitude' phrase refers to clipped-token fractions, "
            "not clip-range widths). "
    ...
    "cispo": AlgorithmSpec(
        ...
        notes=(
            ...
            "bound (1 + clip_ratio_high with clip_ratio_high: 0.2 pinned in actor.yaml, "
            "clip_ratio_low likewise 0.2, hence verl's two-sided clamp; the code falls back to "
            "clip_ratio only if clip_ratio_high is None, which the shipped defaults never "
            "trigger) - replace it to match a tuned setting. "
    ...
    "reinforce_pp": AlgorithmSpec(
        ...
        notes=(
            ...
            "gamma = 1.0 (Sec. 4.2.3). "
            "Advantage: eq. 10 z-normalizes the TOKEN-level advantages of eq. 8 across the "
            "global batch (OpenRLHF flatten-normalizes the masked [B, T] advantages). The "
            "paper-faithful composition is kl_in_reward(kind=K1, coef=0.001), then reward-to-go "
            "at gamma = 1.0 (that [B, T] tensor is eq. 8 exactly), then whiten(returns, "
            "response_mask, shift_mean=True) — equal to eq. 10 up to the variance convention "
            "(whiten uses Bessel-corrected variance with eps inside the sqrt; OpenRLHF uses "
            "population variance with a 1e-8 clamp; the paper specifies neither). "
            "reinforce_pp_advantages(group_ids=None, batch_norm=True) — this spec's "
            "advantage_config — is polgrad's per-sequence simplification over [B] scalar "
            "rewards: it coincides with eq. 10 only at T = 1 and otherwise differs "
            "(token-level statistics weight sequences by length, and the per-token KL-to-go "
            "term never enters a per-sequence reward). "
            "The REINFORCE++-baseline variant (Appendix A) subtracts the per-group mean and "
            "then divides by the batch std = pass group_ids; the paper's variant also uses the "
            "k2 KL estimator ('k2 instead of the k3 divergence approximation method in GRPO', "
            "Appendix A) — this spec keeps the main-text k1-in-reward and does not switch. "
    ...
    "grpo_tis": AlgorithmSpec(
        ...
        notes=(
            ...
            "Cap: PR #2953 as merged shipped tis_imp_ratio_cap: -1 (TIS disabled by default) "
            "and prescribed no tuned value; the PR description's usage example enables C = "
            "10.0 (+actor_rollout_ref.actor.behav_imp_weight_cap=10.0), which is the cap "
            "shipped here - replace to match your setup. verl has since refactored TIS into "
            "the rollout_is config family (compute_policy_loss_vanilla now takes precomputed "
            "rollout_is_weights; default rollout_is_threshold: 2.0). The PR links the "
            "motivating blog 'Your Efficient RL Framework Secretly Brings You Off-Policy RL "
            "Training'. "
